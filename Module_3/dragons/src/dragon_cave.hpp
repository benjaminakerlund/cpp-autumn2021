...
class DragonCave {
public:
    DragonCave() {} //Constructor implemented here already for simpleness
    ~DragonCave(); //Destructor implemented in .cpp
    DragonCave(const DragonCave&) = delete; //Copy constructor - prevented
    void operator=(const DragonCave&) = delete;  //Copy assignment - prevented

    //Member functions
    const std::list<Dragon*> GetDragons() const;
    void Accommodate(Dragon* drake);
