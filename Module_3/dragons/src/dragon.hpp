...
class Dragon {
public:
  /* Constructor definition, implemented in .cpp*/
  Dragon(const std::string& name, size_t age, size_t size);

  /* virtual Destructor not implemented in .cpp */
  virtual ~Dragon() {}
  ...
  /* Virtual functions */
  virtual void Eat(std::list<Food>& food) = 0; //Pure virtual function
  virtual void Hoard(std::list<Treasure>& Treasure) = 0; //pure virtual function

  /* Operator overload << */
  friend std::ostream& operator<<(std::ostream& os, const Dragon& drake);
