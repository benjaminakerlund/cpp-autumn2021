...
class Dragon {
public:
  Dragon(const string& name, size_t age, size_t size);
  ...
  const list<Treasure>& GetTreasures() const;

  virtual void Eat(list<Food>& f) = 0;

  virtual void Hoard(list<Treasure>& t) = 0;

  friend ostream& operator<<(ostream& os, const Dragon& d);

protected:
  string name_;
  size_t age_;
  size_t size_;
  list<Treasure> treasures_;
